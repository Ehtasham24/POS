...
const AddCreditModal = ({ isOpen, onClose, onAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to add receivable entry");
      alert("Receivable entry added!");
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Receivable (Customer)">
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div>
          <label htmlFor="credit_name" className={labelClass}>
            Customer Name
          </label>
          <input
            type="text"
