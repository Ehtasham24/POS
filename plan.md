...
**Web research into publicly available bank/wallet SMS templates (2026-08-16): no template exists publicly, but two useful facts do.** Searched for published Pakistani bank/wallet SMS or email "payment received" templates to avoid needing a real sample from every single bank. None exist — banks deliberately don't publish exact wording (it makes phishing/smishing spoofing easier, and Pakistan has active SMS-fraud targeting bank customers mimicking "wrong transfer, please return" messages from non-official numbers). Two things did come out of the research, worth acting on:
- **Confirmed official shortcodes**: JazzCash sends only from `8558`, Easypaisa only from `3737` — anything claiming to be a payment notification from a different sender (e.g. a normal `03xx` number) is fraudulent. These are exactly the values worth defaulting `Prefs.monitoredSmsSenders` toward once a shop actually configures the SMS channel — both a usability shortcut and a real anti-spoofing measure, since the receiver only ever forwards texts from senders on that allowlist in the first place.
- **A real (if informal) structural clue**: a small third-party script (written by someone parsing their own archived JazzCash texts) confirms real JazzCash "received" SMS contain the word "received," a linked-bank account reference, and an 11-12 digit transaction ID — but *also* that JazzCash's shortcode sometimes sends promotional texts ("download now for free…") mixed in with transactional ones. That's a concrete false-positive risk for `generic.js`'s plain "Rs./PKR + number" match once the SMS channel is live against a real shortcode's full traffic, not just transactional messages — worth guarding against when a JazzCash-specific parser gets tuned (e.g. requiring "received" in the text, not just an amount pattern), not a reason to change anything before real samples exist.

Conclusion unchanged: the fastest real path is still 2-3 real redacted samples per bank/wallet the owner actually uses, once the SMS/email channels are live on the phone — this doesn't shortcut past that, it only removes guesswork from the sender-allowlist defaults and flags one thing to watch for.

**SMS and email added as explicit channels alongside app notifications (2026-08-16).** The owner confirmed they want two specific channels monitored on the phone, not just "whatever app posts a notification" — email and SMS. Reading the actual code first showed the backend (`Controller/paymentNotificationController.js`'s `ReceiveNotification`, `Sevices/PaymentNotifications/parsers/index.js`'s `getParser`) already treats `packageName` as an opaque parser-lookup key with no validation it's a real Android package, and `matchingService.js` is already fully source-agnostic — so **email needed zero code changes**: the owner just ticks their email app (e.g. Gmail) in the already-existing "Apps to Monitor" picker, same as any bank/wallet app. **SMS got one new piece**: `PaymentNotificationForwarder/app/src/main/java/com/possystem/paymentforwarder/SmsReceiver.kt`, a `BroadcastReceiver` on `SMS_RECEIVED` (chosen over reading SMS through the notification banner, which can truncate/group messages) — gated by a new opt-in `Prefs.monitoredSmsSenders` allowlist (mirrors `monitoredPackages`'s explicit-opt-in shape, so personal texts never leave the phone), forwarding to the same `/api/bank-payments/webhook/notification` endpoint with a synthetic `packageName` of `sms:<sender>` so a tuned parser can later be registered per-sender exactly like a per-app one. `MainActivity.kt`/`activity_main.xml`/`strings.xml` gained the runtime SMS-permission request flow and a sender-list UI; `AndroidManifest.xml` gained `RECEIVE_SMS` + the receiver registration. Not built against the Gmail OAuth design (Phase B below stays superseded) — the owner explicitly ruled that back-and-forth (Google Cloud project, consent screen, token storage) out in favor of the simpler notification-based email path. Still unbuilt/unverified for the same reason as the rest of this app: no Android SDK in this session.
